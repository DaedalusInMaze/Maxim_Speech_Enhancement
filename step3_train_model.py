    ...
    for record in range(len(dataset[0])):
        ...
        if epoch and not epoch%10:
            for r in range(len(testset[0])):
                ...
                save_wav(os.path.join(DATADIR,'predicted4', 'noisy' + str(r) + '.wav'), noisy_audio, SAMPLING_RATE)
                save_wav(os.path.join(DATADIR,'predicted4', 'pred' + str(r) + '_epoch_' + str(epoch) + '.wav'), predicted_audio, SAMPLING_RATE)
