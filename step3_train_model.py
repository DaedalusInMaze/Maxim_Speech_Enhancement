    ...
    for record in range(len(dataset[0])):
        ...
        if not datatype : # if the recording is assigned in the training set
            ...
            if epoch and not epoch%2:
                ...
                save_wav(os.path.join(DATADIR,'predicted2', 'noisy' + str(record) + '.wav'), noisy_audio, SAMPLING_RATE)
                save_wav(os.path.join(DATADIR,'predicted2', 'pred' + str(record) + '_epoch_' + str(epoch) + '.wav'), predicted_audio, SAMPLING_RATE)
                save_wav(os.path.join(DATADIR,'predicted2', 'clean' + str(record) + '.wav'), clean_audio, SAMPLING_RATE)
